															...
															foreach ($chklst as $subrow) {
																if($subrow->heading_code == $row->code && $subrow->isheading == 'f'){
																	...
																	<tr class="warning" id="body-part-b-<?php echo $data_no; ?>">
																		...
																		<td>
																			<select class="form-control" name="defects2[]">
																				<option value="N" <?php if(isset($subrow->defect_status)) if(trim($subrow->defect_status) == 'N') { echo 'selected'; } ?>>No</option>
																				<option value="Y" <?php if(isset($subrow->defect_status)) if(trim($subrow->defect_status) == 'Y') { echo 'selected'; } ?>>Yes</option>
																				<option value="NA" <?php if(isset($subrow->defect_status)) if(trim($subrow->defect_status) == 'NA') { echo 'selected'; } ?>>N/A</option>
																			</select>
																		</td>
																		<td>
